...
class MarkdownReporter:
    ...
    def _generate_header(self, prometheus_url: Optional[str] = None) -> list[str]:
        ...
        lines.extend(["**Tool:** prom-slo-analyzer", "", "---"])

        return lines

    def _generate_executive_summary(
        self, services: list[ServiceMetrics], classifications: list[ClassificationResult], gaps: list[GapResult]
    ) -> list[str]:
        """Generate executive summary."""
        lines = ["## Executive Summary"]

        services_with_slis = len([s for s in services if any(c.metric_name in s.metrics for c in classifications)])
        services_with_gaps = len({g.service_name for g in gaps})
    ...
    def _generate_service_detail_section(
        ...
        # Gaps
        if gaps:
            lines.extend(["#### Identified Issues", ""])
    ...
    def _generate_recommendations(self, gaps: list[GapResult]) -> list[str]:
        ...
        if critical_gaps:
            lines.extend(["### Immediate Actions Required", "", "Address these critical gaps first:"])

            for i, gap in enumerate(critical_gaps[:5], 1):  # Top 5 critical
                lines.extend([f"{i}. **{gap.service_name}**: {gap.message}", f"   - {gap.recommendation}", ""])
