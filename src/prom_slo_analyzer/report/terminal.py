...
class TerminalReporter:
    ...
    def report_discovery_summary(self, services: list[ServiceMetrics]) -> None:
        ...
        for service in services:
            ...
            table.add_row(service.name, namespace, str(len(service.metrics)), sample_metrics)

        self.console.print(table)
        self.console.print()

    def report_classification_summary(self, all_classifications: list[ClassificationResult]) -> None:
        ...
        for sli_type, count in type_counts.most_common():
            ...
            table.add_row(sli_type.value.replace("_", " ").title(), str(count), description)

        self.console.print(table)
        self.console.print()

    def report_service_analysis(
        self, service: ServiceMetrics, classifications: list[ClassificationResult], gaps: list[GapResult]
    ) -> None:
        ...
        # Build service info
        info_lines = [
            "[bold blue]Service Analysis[/bold blue]",
        ...
        if classifications:
            ...
            for sli_type, type_classifications in by_type.items():
                ...
                info_lines.append(
                    f"{emoji} {sli_type.value.replace('_', ' ').title()}: {len(type_classifications)} metrics"
                )
    ...
    def report_overall_summary(
        self, services: list[ServiceMetrics], all_classifications: list[ClassificationResult], all_gaps: list[GapResult]
    ) -> None:
        ...
        services_with_gaps = len({g.service_name for g in all_gaps})
    ...
    def _show_classifications_table(self, classifications: list[ClassificationResult]) -> None:
        ...
        for classification in sorted_classifications:
            ...
            table.add_row(classification.metric_name, sli_type_display, confidence_str, promql)

        self.console.print(table)

    def _show_gaps_table(self, gaps: list[GapResult]) -> None:
        ...
        for gap in sorted_gaps:
            ...
            table.add_row(gap.rule_name, severity_display, message, recommendation)
