...
class ErrorRateClassifier(BaseClassifier):
    ...
    def _is_error_counter(self, metric_name: str) -> bool:
        ...
        error_patterns = [
            '_errors_total', '_error_total', '_errors', '_error_count',
            '_failed_total', '_failures_total', '_failure_total',
            '_faults_total', '_fault_total', '_timeouts_total',
            '_retries_total', '_retry_total'
        ]
    ...
    def _is_http_status_metric(self, metric_name: str) -> bool:
        ...
        http_patterns = [
            'http_requests_total', 'http_request_total', 'requests_total',
            'http_responses_total', 'response_total', 'status_code'
        ]
    ...
    def _is_exception_metric(self, metric_name: str) -> bool:
        ...
        exception_patterns = [
            'exceptions_total', 'exception_total', 'panics_total',
            'crashes_total', 'abort_total', 'rejected_total'
        ]
    ...
    def _classify_error_counter(self, metric_name: str, metadata: Optional[Dict[str, Any]]) -> ClassificationResult:
        ...
        positive_patterns = [
            'errors', 'failed', 'failures', 'faults', 'timeouts', 'retries'
        ]
    ...
    def _classify_http_status(self, metric_name: str, metadata: Optional[Dict[str, Any]]) -> ClassificationResult:
        ...
        suggested_promql = (
            f'rate({metric_name}{{status=~"5.."}[5m]) / '
    ...
    def _classify_exception(self, metric_name: str, metadata: Optional[Dict[str, Any]]) -> ClassificationResult:
        """Classify exception metric for error rate SLI."""
        
        positive_patterns = [
            'exceptions', 'panics', 'crashes', 'abort', 'rejected'
        ]
    ...
    def _extract_base_name(self, metric_name: str) -> str:
        ...
        error_suffixes = [
            ...
            '_failed_total', '_failures_total', '_failure_total',
            '_faults_total', '_fault_total', '_timeouts_total',
            '_retries_total', '_retry_total', '_exceptions_total'
        ]
