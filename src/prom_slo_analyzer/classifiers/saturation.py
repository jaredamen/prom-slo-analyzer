...
class SaturationClassifier(BaseClassifier):
    ...
    def _classify_resource_utilization(
        self, metric_name: str, metadata: Optional[dict[str, Any]]
    ) -> ClassificationResult:
        """Classify resource utilization metric for saturation SLI."""

        # High confidence for resource utilization
        positive_patterns = ["cpu", "memory", "disk", "network", "bandwidth", "io", "usage", "utilization", "percent"]
    ...
    def _classify_capacity_metric(self, metric_name: str, _metadata: Optional[dict[str, Any]]) -> ClassificationResult:
        """Classify capacity metric for saturation SLI."""

        positive_patterns = ["capacity", "usage", "utilization", "used", "consumed", "limit", "quota", "allocation"]

        negative_patterns = ["available", "free", "remaining"]  # These are inverse saturation metrics
    ...
    def _classify_pool_metric(self, metric_name: str, _metadata: Optional[dict[str, Any]]) -> ClassificationResult:
