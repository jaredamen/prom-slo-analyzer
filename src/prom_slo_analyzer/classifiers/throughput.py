...
class ThroughputClassifier(BaseClassifier):
    ...
    def _is_request_counter(self, metric_name: str) -> bool:
        ...
        request_patterns = [
            '_requests_total', '_request_total', '_requests',
            'http_requests_total', 'grpc_requests_total',
            '_calls_total', '_call_total', '_invocations_total',
            '_hits_total', '_accesses_total'
        ]
    ...
    def _is_processing_counter(self, metric_name: str) -> bool:
        ...
        processing_patterns = [
            '_processed_total', '_processed', '_handled_total',
            '_completed_total', '_finished_total', '_executed_total',
            '_messages_total', '_events_total', '_items_total',
            '_operations_total', '_tasks_total'
        ]
    ...
    def _is_transaction_counter(self, metric_name: str) -> bool:
        ...
        transaction_patterns = [
            '_transactions_total', '_transaction_total',
            '_orders_total', '_purchases_total', '_payments_total',
            '_uploads_total', '_downloads_total', '_queries_total'
        ]
    ...
    def _classify_request_counter(self, metric_name: str, metadata: Optional[Dict[str, Any]]) -> ClassificationResult:
        ...
        positive_patterns = [
            'requests', 'calls', 'invocations', 'hits', 'accesses'
        ]
    ...
    def _classify_processing_counter(self, metric_name: str, metadata: Optional[Dict[str, Any]]) -> ClassificationResult:
        """Classify processing counter for throughput SLI."""
        
        positive_patterns = [
            'processed', 'handled', 'completed', 'finished', 'executed',
            'messages', 'events', 'items', 'operations', 'tasks'
        ]
    ...
    def _classify_transaction_counter(self, metric_name: str, metadata: Optional[Dict[str, Any]]) -> ClassificationResult:
        """Classify transaction counter for throughput SLI."""
        
        positive_patterns = [
            'transactions', 'orders', 'purchases', 'payments',
            'uploads', 'downloads', 'queries'
        ]
