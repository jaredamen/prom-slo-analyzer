...
class ServiceMetrics:
    ...
    metrics: List[str]
    labels: Dict[str, Set[str]]  # label_name -> set of values
...
class ServiceDiscovery:
    ...
    def _is_system_metric(self, metric_name: str) -> bool:
        ...
        """
        system_prefixes = [
            'prometheus_', 'go_', 'process_', 'node_', 'container_',
            'kubernetes_', 'kube_', 'cadvisor_', 'up', 'scrape_',
            'net_conntrack_', 'rate_', 'increase_', 'histogram_quantile'
        ]
    ...
    def _is_generic_term(self, term: str) -> bool:
        ...
        """
        generic_terms = {
            'http', 'https', 'tcp', 'udp', 'grpc', 'rest', 'api',
            'request', 'requests', 'response', 'responses',
            'error', 'errors', 'status', 'health', 'up', 'down',
            'latency', 'duration', 'time', 'seconds', 'milliseconds',
            'count', 'total', 'sum', 'avg', 'max', 'min',
            'bytes', 'bits', 'rate', 'ratio', 'percent', 'gauge',
            'histogram', 'summary', 'counter', 'metric'
        }
