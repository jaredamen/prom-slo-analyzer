...
from .report import MarkdownReporter, TerminalReporter
from .rules import MissingErrorRateRule, MissingLatencyRule, NoSLICandidatesRule
...
def scan(
    ...
    namespace: Optional[str] = typer.Option(None, "--namespace", "-n", help="Filter services by namespace"),
    format: str = typer.Option("terminal", "--format", "-f", help="Output format: terminal, markdown"),
    output: Optional[str] = typer.Option(None, "--output", "-o", help="Output file (only for non-terminal formats)"),
) -> None:
    ...
    if not prometheus_url:
        console.print(
            "[red]Error: Prometheus URL is required. Use --prometheus-url or set PROMETHEUS_URL environment variable.[/red]"
        )
        raise typer.Exit(1)
    ...
    try:
        ...
        rules = [MissingLatencyRule(), MissingErrorRateRule(), NoSLICandidatesRule()]
        ...
        if format == "terminal":
            ...
            report_content = markdown_reporter.generate_report(services, all_classifications, all_gaps, prometheus_url)
            if output:
...
def discover(
    ...
    try:
        ...
        with Progress(
            ...
            progress.add_task(description="Connecting to Prometheus...", total=None)
        ...
        with Progress(
            ...
            progress.add_task(description="Discovering services...", total=None)
            services = discovery.discover_services(namespace_filter=namespace)
