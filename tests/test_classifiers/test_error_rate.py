...
from prom_slo_analyzer.classifiers.error_rate import ErrorRateClassifier
...
class TestErrorRateClassifier:
    ...
    def test_classify_non_error_metric(self, classifier):
        ...
        # Success counter (should be classified as HTTP metric, which is correct)
        result = classifier.classify("user_login_attempts_total")
    ...
    def test_classify_with_metadata(self, classifier):
        ...
        metadata = {"type": "counter", "help": "Total number of failed requests"}
    ...
    def test_confidence_levels(self, classifier):
        ...
        assert direct_confidence >= 0.6
        assert http_confidence >= 0.7
        assert exception_confidence >= 0.6
        assert direct_confidence >= http_confidence or abs(direct_confidence - http_confidence) <= 0.11
